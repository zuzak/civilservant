...
describe('buttery module', function () {
  ...
  it('should initalise with zero invocations', function () {
    assert.equal(mockBot.runCommand('!bcount'), 'Count since last reload: 0')
  })

  it('should count the number of invocations', function () {
    mockBot.runCommand('!buttery')
    assert.equal(mockBot.runCommand('!bcount'), 'Count since last reload: 1')
  })

  it('should toggle off', function () {
    assert.equal(mockBot.runCommand('!acid'), 'disabled base')
  })

  it('should toggle on', function () {
    assert.equal(mockBot.runCommand('!acid'), 'enabled base')
  })
